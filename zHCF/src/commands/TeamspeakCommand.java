...
public class TeamspeakCommand implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args){
		...
		if(!sender.hasPermission("zhcf.teamspeak")){
			sender.sendMessage(ChatColor.translateAlternateColorCodes('&', Main.getInstance().config.getString("messages.nopermission")));
			return true;
		}
		for(String teamspeakcmd : Main.getInstance().config.getStringList("teamspeak")){
			p.sendMessage(ChatColor.translateAlternateColorCodes('&', teamspeakcmd));
		}
		return false;
	}
}
